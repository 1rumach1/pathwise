...
export class UIManager {
  ...
  setupEventListeners() {
    ...
    // View controls
    document.getElementById('view-all').addEventListener('click', () => this.changeView('all'));
    document.getElementById('view-today').addEventListener('click', () => this.changeView('today'));
    document.getElementById('view-upcoming').addEventListener('click', () => this.changeView('upcoming'));
    ...
    // Task details modal
    document.getElementById('close-details').addEventListener('click', () => this.closeTaskDetailsModal());
    document.getElementById('edit-task').addEventListener('click', () => this.editCurrentTask());
    document.getElementById('delete-task').addEventListener('click', () => this.deleteCurrentTask());
  ...
  changeView(view) {
    ...
    // Update map for today view
    if (view === 'today') {
      this.mapManager.updateTaskMarkers(this.taskManager.getTodayTasks());
      this.mapManager.updateRoutes(this.taskManager.getTodayTasks());
    } else {
      // For other views, still show today's tasks on the map
      this.mapManager.updateTaskMarkers(this.taskManager.getTodayTasks());
      this.mapManager.updateRoutes(this.taskManager.getTodayTasks());
    }
  }
  ...
  handleSearch() {
    ...
    // Search tasks
    const searchResults = this.taskManager.searchTasks(query);
    
    // Render results
    this.renderTaskList(searchResults);
  }
  ...
  showTaskDetails(taskId) {
    ...
    document.getElementById('detail-start').textContent = `${formatDate(task.startTime)} at ${formatTime(task.startTime)}`;
    document.getElementById('detail-end').textContent = `${formatDate(task.endTime)} at ${formatTime(task.endTime)}`;
  ...
  editCurrentTask() {
    ...
    const locationMatch = this.locations.find(loc => 
